...
def getTag(n):
    global origStr
    theLine = origStr[n]
    indexLeft = theLine.index("[")+1
    indexRight = theLine.index("]",indexLeft)
    term = theLine[indexLeft:indexRight]
    return term
...
def drawFrame(frameNum,paragraph,emotion,imageNum,pose,phoneNum,poseTimeSinceLast,poseTimeTillNext):
    ...
    if imageNum == CACHES[2][0]:
        ...
        scribble = Image.open("lookuptest/"+str(getTag(imageNum)).lower()+".png")
...
for frame in range(0,FRAME_COUNT):
    ...
    print("Just drew frame "+str(frame)+" / "+str(FRAME_COUNT))
